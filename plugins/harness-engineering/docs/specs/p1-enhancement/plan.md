...
### 현재 상태
- `hooks/lib/review-engine.sh`에 `verify_spec_compliance()`와 `spawn_subagent_for_review()`가 구현되어 있음
- `hooks/__tests__/review-engine.test.sh`로 회귀 테스트가 존재함
- 후속 과제: 판정 정확도와 실제 리뷰 결과 수집 흐름 보강
...
- `hooks/lib/crash-recovery.sh`에 stuck 감지와 recovery 로직이 구현되어 있음
...
### 현재 상태
- `hooks/lib/browser-testing.sh`와 대응 테스트 스위트가 존재함
- 프로젝트별 설치/리포트 연동은 타깃 환경에 따라 추가 조정이 필요함
